 ...
 */
t_log* log_create(t_log* logger, char* file, char *program_name, bool is_active_console, t_log_level detail) {
...
static void log_write_in_level(t_log* logger, t_log_level level, const char* message_template, va_list list_arguments) {
	...
	if (isEnableLevelInLogger(logger, level)) {
		...
		char message[LOG_MAX_LENGTH_MESSAGE + 1],buffer[LOG_MAX_LENGTH_BUFFER + 1];
		//char *message, *buffer;
		...
		char str_time[] = "hh:mm:ss:mmmm";
		char time[strlen(str_time)+ 1];

		temporal_get_string_time(str_time, time);
